  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      <input
        ...
        className="flex-1 bg-muted/30 border border-border rounded-lg px-3 py-1.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#00e676]/40"
        maxLength={80}
      />
      ...
      <button onClick={onCancel} className="p-1.5 rounded-lg text-muted-foreground/70 hover:text-foreground hover:bg-muted/50 transition-colors"><XIcon className="w-4 h-4" /></button>
    </div>
  );
}
      ...
      <PageTransition className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground/70 mt-0.5">Alerts and saved filter configurations.</p>
          </div>
        </div>
        ...
        <div className="flex border-b border-border/50 mb-6">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${tab === "watches" ? "border-[#00e676] text-[#00e676]" : "border-transparent text-muted-foreground/70 hover:text-foreground/80"}`}
          >
            <Bell className="w-4 h-4" />
            Watches
            {watches.length > 0 && <span className="text-xs bg-muted/30 px-1.5 py-0.5 rounded-full">{watches.length}</span>}
          </button>
          <button
            onClick={() => setTab("saved")}
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${tab === "saved" ? "border-[#00e676] text-[#00e676]" : "border-transparent text-muted-foreground/70 hover:text-foreground/80"}`}
          >
            <Bookmark className="w-4 h-4" />
            Saved Searches
            {searches.length > 0 && <span className="text-xs bg-muted/30 px-1.5 py-0.5 rounded-full">{searches.length}</span>}
          </button>
        </div>
        ...
        {loading ? (
          ...
          watches.length === 0 ? (
            <div className="bg-card border border-border/50 rounded-2xl p-12 text-center">
              <BellOff className="w-10 h-10 text-muted-foreground/50 mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-foreground/80 mb-2">No watches yet</h3>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-card border border-border hover:border-[#00e676]/40 hover:text-[#00e676] text-foreground/80 text-sm font-medium px-4 py-2.5 rounded-xl transition-colors"
                >
            ...
            <div className="space-y-6">
              {Object.entries(grouped).map(([type, items]) => {
                ...
                return (
                  <div key={type} className="bg-card border border-border/50 rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
                      <Icon className="w-4 h-4" style={{ color }} />
                      <h2 className="text-sm font-semibold">{TYPE_LABELS[type] ?? type}</h2>
                      <span className="ml-auto text-xs text-muted-foreground/70 bg-muted/30 px-2 py-0.5 rounded-full">{items.length}</span>
                    </div>
                      ...
                      {items.map((watch) => (
                        <div key={watch.id} className="flex items-center justify-between px-5 py-3.5 group">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-foreground">{watch.watchValue}</span>
                          </div>
                          <div className="flex items-center gap-3">
          ...
          searches.length === 0 ? (
            <div className="bg-card border border-border/50 rounded-2xl p-12 text-center">
              <Bookmark className="w-10 h-10 text-muted-foreground/50 mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-foreground/80 mb-2">No saved searches yet</h3>
              <p className="text-sm text-muted-foreground/70 mb-6 max-w-xs mx-auto">
                  ...
                  <div className="px-5 py-4">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                      {editingId === s.id ? (
                        ...
                        <>
                          <h3 className="text-sm font-semibold text-foreground leading-snug">{s.name}</h3>
                          <button
                            onClick={() => setEditingId(s.id)}
                            className="p-1.5 rounded-lg text-muted-foreground/50 hover:text-foreground/80 hover:bg-muted/50 transition-colors shrink-0"
                            title="Rename"
                          >
                    ...
                    {/* Filter summary */}
                    <p className="text-xs text-muted-foreground/70 mb-3">{filterSummary(s.filters)}</p>

                    {/* Dates */}
                    <div className="flex items-center gap-4 text-[11px] text-muted-foreground/50 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {deleteConfirm === s.id ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => setDeleteConfirm(null)} className="px-2.5 py-1.5 rounded-lg text-xs text-muted-foreground/70 hover:text-foreground hover:bg-muted/50 transition-colors">
                            Cancel
                          </button>
                        </div>
                      ) : (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-muted-foreground/70 hover:text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all"
                        >
